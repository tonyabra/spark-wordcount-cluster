...
while read p; do
  ...
  if [ $n -ge $1 ]; then
    ...
    echo "Unzipping file $n"
    gunzip ./*.wet.gz

    echo "Changing file name"
    printf -v j "%04d" $n
    echo "New file: CC-MAIN-20150124161055-0$j-ip-10-180-212-252.ec2.internal.warc.wet"
    mv *.wet CC-MAIN-20150124161055-0$j-ip-10-180-212-252.ec2.internal.warc.wet

    echo "Pushing to HDFS"
    hdfs dfs -copyFromLocal *.wet /data/
